...
import { incomeCategoryRowData } from "../../data/incomeData";

const IncomeCategoryTable = ({
  incomeCategoryTableCol,
  incomeCategoryTableRowData,
  rowTemplate,
}) => {
  return (
    <table className="w-[90%] mt-10 overflow-y-auto">
      <thead className="text-gray-500">
        <tr>
          {incomeCategoryTableCol?.map((item, i) => (
            <th
              key={i}
      ...
      <tbody className="">
        {incomeCategoryRowData?.map((item, i) => rowTemplate(item, i))}
      </tbody>
    </table>
  );
};
