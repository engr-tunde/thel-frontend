...
import { fetchAllWarehouse} from "../../api";
import Loader from "../../component/global/Loader";
import { ViewWarehouse, warehouseTableColumn } from "../../data/warehouse";
import warehouseRowTemplate from "../../component/warehouse/warehouseRowTemplate";

const WarehouseListPage = () => {
  const [viewWarehouse, setviewWarehouse] = useState(false);

  const { data, loading, error, mutate } = fetchAllWarehouse();
    ...
    <div className="mt-8 w-full flex flex-col">
      ...
      <div className="flex w-full justify-between items-center mt-10 mb-10">
        ...
        <div className="flex items-center justify-center gap-[4px] relative">
          ...
          <div
            onMouseLeave={handleViewWarehouse}
            className={
              viewWarehouse
      ...
      {data ? (
        <Table
          tableColumn={warehouseTableColumn}
          rowData={data?.data}
          rowTemplate={warehouseRowTemplate}
          checkAll={true}
        />
      ) : loading ? (
        <Loader />
